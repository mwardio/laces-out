 ...
 * (`132:slot:16` — an accepted yards-allowed tier rule, but excluded from the emitted profile while
 * D/ST is unsupported) and a bare D/ST-only unsupported stat (`205`, ESPN's "Defensive 2pt
 * Return").
...
describe("currentManagedProjectionProfileKey", () => {
  ...
  it("prices a yards-allowed tier rule when D/ST is the supported position", async () => {
    // This package's private `availableComponents` union spreads
    // `firstPartyTeamDefenseProjectionComponents()`, so the nine yards-allowed tiers must be
    // available here without edits. If they were not, COMPONENT_UNAVAILABLE would withhold D/ST
    // and this key would be null.
