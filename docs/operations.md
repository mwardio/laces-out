...
- **Download my data** calls authenticated `GET /v1/account/export` and downloads a versioned JSON
  ...
  envelopes, push endpoints/keys, and share capabilities. Export schema version 2 records canonical
  league roles as `owner`, `commissioner`, or `member`; version 1 exports may contain the retired
  `manager` and `viewer` values.
...
Deletion transfers an owned league with surviving members to a commissioner first, then a member,
with oldest membership as the tie-break. It deletes a sole-member league. Private
...
joins a later connector as a member, every joined member can refresh shared state, and monotonic
